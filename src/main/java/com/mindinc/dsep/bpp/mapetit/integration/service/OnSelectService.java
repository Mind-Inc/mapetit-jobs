...
import com.mindinc.dsep.bpp.mapetit.rest.beckn.model.select.SelectMessage;
...
public class OnSelectService {
    ...
    public void execute(SelectRequest selectRequest) throws JsonProcessingException {
        Objects.requireNonNull(selectRequest);

        var jobId = Optional.of(selectRequest)
                .map(SelectRequest::getMessage)
                .map(SelectMessage::getOrder)
                .map(Order::getItems)
        ...
        var context = ResponseBuilder.buildContext(selectRequest.getContext(), ContextAction.ON_SELECT.value());
        var order = buildOrder(job);
        var requestBody = OnSelectMessage.builder()
                .order(order)
                .build();
        var url = context.getBapUri().concat(ContextAction.ON_SELECT.value());
    ...
    private Order buildOrder(Job job) {
        var locationId = UUID.randomUUID().toString();
        Order order = new Order();
        order.setProvider(jobToProvider(job, locationId));
    ...
    private Provider jobToProvider(Job job, String locationId) {
        ...
        provider.setFulfillments(
                Arrays.stream(Job.Type.values())
                        .map(type -> Fulfillment.builder().type(job.getType().name()).build())
                        .toList()
        );
